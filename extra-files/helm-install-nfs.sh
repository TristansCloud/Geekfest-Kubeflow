...
# Before this script can be run you
# need to have kubectl setup and 
# connecting to a k8s cluster.
# Modify the NFS_SERVER variable.
...
# Below is if this code is being run on the NFS server.
# NFS_SERVER=`ip route get 8.8.8.8 | awk -F"src " 'NR==1{split($2,a," ");print a[1]}'`
